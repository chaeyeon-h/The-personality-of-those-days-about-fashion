...
class JoinPage:
    ...
    def join(self,values):
        if len(self.db.readData("user",["email"],[self.myEmail]))==0 or len(self.db.readData("user",["name","birth"],[self.myName,self.myBirth]))==0:
            self.db.insertData("user", self.db.rows, values) 
